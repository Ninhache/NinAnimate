...
export const getLinesData = (props: {
  value: string;
  ...
  const ensureUnique = (key: string) =>
    usedKeys[key] ? key + ++usedKeys[key] : key;
...
export default function CodeAnimate(props: {
  ...
  renderSpecialLine: (props: { line: string }) => React.JSX.Element;
  maxAnchor?: number;
  ...
  const linesData = useMemo(
    () => getLinesData({ value, getKey }),
    [value, getKey]
  );
  const highlightedLines = useMemo(
    () => getHighlightedLines({ value, grammar, language }),
    [value]
  );
